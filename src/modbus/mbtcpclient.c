...
/*生成MBAP报文头*/
static uint16_t CreateMbapHeadPart(int DevNo, TCPLocalClientType *client,uint8_t * mbapHead);
/* 向TCP客户端添加TCP服务器列表节点 */
static void AddTCPServerNode(TCPLocalClientType *client,TCPAccessedServerType *server);
/*接收到返回信息后，判断是否是发送命令列表中命令的返回信息，*/
static int FindCommandForRecievedMessage(int DevNo, TCPLocalClientType *client,uint8_t *recievedMessage);
/*处理读从站状态量返回信息，读线圈状态位0x01功能码*/
static void HandleReadCoilStatusRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMesasage,uint16_t startAddress,uint16_t quantity);
/*处理读从站状态量返回信息，读输入状态位0x02功能码*/
static void HandleReadInputStatusRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMesasage,uint16_t startAddress,uint16_t quantity);
/*处理读从站寄存器值的返回信息，读保持寄存器0x03功能码）*/
static void HandleReadHoldingRegisterRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMesasage,uint16_t startAddress,uint16_t quantity);
/*处理读从站寄存器值的返回信息，读输入寄存器0x04功能码*/
static void HandleReadInputRegisterRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMesasage,uint16_t startAddress,uint16_t quantity);

void (*HandleServerRespond[])(int, TCPLocalClientType *client,uint8_t *,uint16_t,uint16_t)={HandleReadCoilStatusRespond,
HandleReadInputStatusRespond,
HandleReadHoldingRegisterRespond,
HandleReadInputRegisterRespond};

/*生成访问服务器的命令*/
uint16_t CreateAccessServerCommand(int DevNo, TCPLocalClientType *client,ObjAccessInfo objInfo,void *dataList,uint8_t *commandBytes)
{
  uint16_t commandLength=0;
  
  CreateMbapHeadPart(DevNo, client,commandBytes);
  ...
  {
    commandLength=SyntheticReadWriteTCPServerCommand(DevNo, objInfo,NULL,NULL,commandBytes);
  }
  ...
  if((objInfo.functionCode==WriteSingleCoil)||(objInfo.functionCode==WriteMultipleCoil))
  {
    ...
    commandLength=SyntheticReadWriteTCPServerCommand(DevNo, objInfo,statusList,NULL,commandBytes);
  }
  ...
  if((objInfo.functionCode==WriteSingleRegister)||(objInfo.functionCode==WriteMultipleRegister))
  {
    ...
    commandLength=SyntheticReadWriteTCPServerCommand(DevNo, objInfo,NULL,registerList,commandBytes);
  }
...
/*解析收到的服务器相应信息*/
void ParsingServerRespondMessage(int DevNo, TCPLocalClientType *client,uint8_t *recievedMessage)
{
  ...
  TCPAccessedServerType *currentServer=client->pServerList;
  if(currentServer == NULL){
		log("------>currentServer == NULL\n");
		return ;
	}

  // int cmdIndex=FindCommandForRecievedMessage(DevNo, client,recievedMessage);
  // if((cmdIndex<0))      //没有对应的请求命令，事务号不相符
  // {
  //   return;
  // }
  int cmdIndex = 0;//一个设备只有一个子站
  
  if((recievedMessage[2]!=0x00)||(recievedMessage[3]!=0x00)) //不是Modbus TCP协议
  {
    perror("this is not tcp protocol\n");
    return;
  }
  ...
  {
    perror("data len is not equal\n");
    return;
  }
  FunctionCode fuctionCode=(FunctionCode)recievedMessage[7];
  if(fuctionCode!=currentServer->pReadCommand[cmdIndex][7])
  {
    perror("rec fuctionCode(0x:%02x) != send fuctionCode(0x:%02x)\n",fuctionCode,currentServer->pReadCommand[cmdIndex][7]);
    return;
  }
...
/*发送命令后，向已发送命令列表中添加命令*/
void AddCommandBytesToList(int DevNo, TCPLocalClientType *client,uint8_t *commandBytes)
{
  TCPAccessedServerType *currentServer=client->pCurrentServer;
...
static void HandleReadCoilStatusRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMessage,uint16_t startAddress,uint16_t quantity)
{
  bool coilStatus[256];
  
  TransformClientReceivedData(DevNo, receivedMessage+6,quantity,coilStatus,NULL);
  
  uint8_t serverAddress=client->pCurrentServer->ipAddress.ipSegment[3];
  
  client->pUpdateCoilStatus(DevNo, serverAddress,startAddress,quantity,coilStatus);
}

/*处理读服务器状态量返回信息，读输入状态位0x02功能码*/
static void HandleReadInputStatusRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMessage,uint16_t startAddress,uint16_t quantity)
{
  bool inputStatus[256];
  
  TransformClientReceivedData(DevNo, receivedMessage+6,quantity,inputStatus,NULL);
  
  uint8_t serverAddress=client->pCurrentServer->ipAddress.ipSegment[3];
  
  client->pUpdateInputStatus(DevNo, serverAddress,startAddress,quantity,inputStatus);
}

/*处理读服务器寄存器值的返回信息，读保持寄存器0x03功能码）*/
static void HandleReadHoldingRegisterRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMessage,uint16_t startAddress,uint16_t quantity)
{
  uint16_t holdingRegister[256];
  TransformClientReceivedData(DevNo, receivedMessage+6,quantity,NULL,holdingRegister);
  uint8_t serverAddress=client->pServerList->pReadCommand[0][6];
  client->pUpdateHoldingRegister(DevNo, serverAddress,startAddress,quantity,holdingRegister);
}

/*处理读服务器寄存器值的返回信息，读输入寄存器0x04功能码*/
static void HandleReadInputRegisterRespond(int DevNo, TCPLocalClientType *client,uint8_t *receivedMessage,uint16_t startAddress,uint16_t quantity)
{
  uint16_t inputRegister[125];
  
  TransformClientReceivedData(DevNo, receivedMessage+6,quantity,NULL,inputRegister);
  
  uint8_t serverAddress=client->pCurrentServer->ipAddress.ipSegment[3];
  
  client->pUpdateInputResgister(DevNo, serverAddress,startAddress,quantity,inputRegister);
...
/* 实例化TCP服务器对象 */
void InstantiateTCPServerObject(
                                TCPAccessedServerType *server,          //要实例化的服务器对象
...
void ModifyWriteTCPServerEnableFlag(int DevNo, TCPLocalClientType *client,uint8_t ipAddress,bool en)
{
...
/* 获得服务器的写使能标志位的状态 */
bool GetWriteTCPServerEnableFlag(int DevNo, TCPLocalClientType *client,uint8_t ipAddress)
{
  bool status=false;
...
/* 判断当前是否有写操作使能 */
bool CheckWriteTCPServerNone(int DevNo, TCPLocalClientType *client)
{
  bool status=true;
