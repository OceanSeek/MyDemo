...
int ModbusTcp_Deal(int DevNo, uint8_t *pbuf, uint16_t len);
int ModbusTcp_Slaver_Receive(int DevNo, uint8_t *buf, uint16_t len);
int ModbusTcp_Slaver_OnTimeOut(int DevNo);
int ModbusTcp_Slaver_Send(int DevNo,char *buf, int len);


int Init_ModbusTcpSlaver(int DevNo)
{

	gpDevice[DevNo].Receive = ModbusTcp_Slaver_Receive;
//	gpDevice[DevNo].Task = GX101Slaver_Task;
	gpDevice[DevNo].OnTimeOut = ModbusTcp_Slaver_OnTimeOut;
...
int ModbusTcp_Slaver_Receive(int DevNo, uint8_t *buf, uint16_t len)
{
    ...
    while(BufTemp<buf+len){
        ...
        if(ret != RET_ERROR){
			...
			DumpHEX(buf, len);
            ModbusTcp_Deal(DevNo, BufTemp, LenTmp);
        }
        BufTemp+=LenTmp;
        LenRemain-=LenTmp;
    }
...
int ModbusTcp_Deal(int DevNo, uint8_t *pbuf, uint16_t len)
{
	...
	// SendLen = ParsingServerRespondMessage(DevNo, pbuf, gpDevice[DevNo].pSendBuf, len, gpDevice[DevNo].Address);
	LogSysLocalTime();
	log("Dev %d:slaver send:",DevNo);
	DumpHEX(gpDevice[DevNo].pSendBuf, SendLen);
	ModbusTcp_Slaver_Send(DevNo, gpDevice[DevNo].pSendBuf, SendLen);
	switch(pFun)
	{
		case 1:		//����Ȧ	
			break;
